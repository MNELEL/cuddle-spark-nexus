...
**עדכון אחרון:** 5 באוגוסט 2026 — נוסף סעיף 10: בדיקה מול קוד חי של רשימת פערים/בקשות פיתוח חדשה שהתקבלה בשיחה נפרדת (SecuritySettings, ReminderPreferencesCard, handoff PDF, audit log לרולאובר, trial gate). כל הממצאים אושרו מול קוד בפועל; לא בוצע שינוי קוד — רק תיעוד ותכנון.
...
| # | פער | סטטוס מאומת | פרטים |
|---|---|---|---|
| 1 | תאריך עדכון אחרון בדוח מסירה (handoff PDF) | ❌ חסר בפועל, תיקון קטן | `listClassProfiles` **כבר מחזיר `updated_at`** מה-DB (התגלה בבדיקה — לא היה ידוע). החוסר הוא רק ב-type `HandoffProfile` וב-`buildHandoffPdfBlob` שלא מקבלים/מדפיסים את השדה. אין צורך ב-migration, רק חיווט. |
| 2 | התראה למורה על ארכוב כיתה | ❌ לא קיים | קיים רק `toast.success` עצמי למבצע הפעולה. אין מנגנון notification חוצה-משתמשים (למשל למורה שיורש כיתה). |
| 3 | Audit log ייעודי לרולאובר (מעבר שנה) | ❌ לא קיים | קיים audit log רק לניהול מוסדות/תפקידים (admin). אין טבלה/רשומה ייעודית לפעולת מעבר-שנה עצמה. |
| 4 | בדיקות RLS/רולאובר אוטומטיות (test suite) | ❌ לא קיים | אין אף קובץ `*.test.ts`/`*.spec.ts` בפרויקט. RLS ל-`classes`/`student_profiles` מתוקן ומאומת **ידנית** בלבד (`private.is_institution_admin` כולל principal + בדיקת `institution_id`). |
| 5 | חיבור trial ל-UI | ❌ "יתום" | `getMyTrialStatus` (server function) קיים אך לא בשימוש בשום route תחת `src/routes`. אין מסך שמסביר הפעלת ניסיון, ואין gate שחוסם תוכן/כלים חינמיים למשתמשים לא-רשומים. |
| 6 | כפתור Retry בכשל שמירה — SecuritySettings | ❌ לא קיים | אושר בקריאת קוד: יש רק `toast.error` + `setErr` מקומי. אין UI לנסיון חוזר בלי סגירת הדיאלוג/ריענון. |
| 7 | כפתור Retry + עדכון אופטימי — ReminderPreferencesCard | ❌ לא קיים | `useMutation` רגיל, ללא `onMutate` שמעדכן state לפני תשובת השרת וללא rollback בכשל. הודעת השגיאה גנרית ("שמירת ההעדפות נכשלה"), בלי זמן תגובה משוער. |
...
### 10.3 בקשות פיתוח חדשות (טרם מיושמות) — מתועדות לצורך תכנון קרדיטים

**קבוצה A — פיצ'רים חדשים:**
...
**קבוצה B — תיקוני UX (קטנים, ממוקדי קובץ):**
5. Retry ב-SecuritySettings + ReminderPreferencesCard (ראה 10.1 #6, #7)
6. Toast מותאם הקשר לכל סוג שינוי, עם זמן תגובה משוער
7. עדכון אופטימי + rollback ב-ReminderPreferencesCard (ראה 10.1 #7)

**קבוצה C — השלמות אבטחה/ממשל (בינוניות):**
8. תאריך עדכון בדוח מסירה + מסך העברה (ראה 10.1 #1 — תיקון קטן, השדה כבר קיים ב-DB)
9. מנגנון התראה בין-משתמשי לארכוב כיתה (ראה 10.1 #2)
10. Audit log לרולאובר (ראה 10.1 #3)
11. Test suite ל-RLS ולהעתקת תלמידים (ראה 10.1 #4)
12. חיבור ה-trial הקיים למסך הרשמה + gate לתוכן חינמי (ראה 10.1 #5)

אף אחת מהבקשות בקבוצות A–C טרם מומשה. יש לתכנן ולתעדף לפני התחלת עבודה, ולהעדיף העלאה ישירה ל-GitHub על פני `send_message` היכן שהשינוי לא דורש migration שרץ אוטומטית.
