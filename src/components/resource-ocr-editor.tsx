...
function confidenceLabel(c: number | undefined) {
  ...
  if (c >= 0.85) return { text: `ודאות גבוהה · ${pct}%`, cls: "text-emerald-600 dark:text-emerald-400" };
  if (c >= 0.6) return { text: `ודאות בינונית · ${pct}%`, cls: "text-amber-600 dark:text-amber-400" };
  return { text: `ודאות נמוכה · ${pct}% — כדאי לעבור על הטקסט`, cls: "text-rose-600 dark:text-rose-400" };
}
    ...
    <Dialog open={open} onOpenChange={(v) => { if (!v) onClose(); }}>
      <DialogContent className="max-w-3xl max-h-[92vh] overflow-y-auto" dir="rtl">
        ...
        <p className="text-xs text-muted-foreground">
          אפשר לתקן כאן שגיאות OCR. אחרי השמירה הטקסט מאונדקס מחדש והחיפוש בתוך המסמך יעבוד לפי הגרסה המתוקנת.
        </p>
        <Textarea
          dir="rtl"
          rows={18}
        ...
        <DialogFooter className="flex-wrap gap-2">
          <Button variant="outline" disabled={rescanMut.isPending} onClick={() => rescanMut.mutate()}>
            {rescanMut.isPending
