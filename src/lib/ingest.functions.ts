...
export const commitRoster = createServerFn({ method: "POST" })
  ...
  .handler(async ({ data, context }) => {
    ...
    // existing roster, for field-level merge instead of blind insert
    ...
    for (const s of data.students) {
      ...
      if (matchId) {
        ...
        const { data: ins, error } = await context.supabase
          .from("students")
          .insert({ class_id: data.class_id, ...fields } as never)
          .select("id")
          .single();
        if (error) { console.error("[DB]", error); throw new Error("הפעולה נכשלה."); }
        inserted += 1;
...
export const commitResource = createServerFn({ method: "POST" })
  ...
  .inputValidator((d) => z.object({
    ...
    resource_type: z.enum(RESOURCE_TYPES as [string, ...string[]]).default("worksheet"),
...
async function analyzeAuto(
  ...
  const system = `אתה עוזר של רב/מלמד בתלמוד תורה חרדי במערכת הכיתה שלי. קיבלת קובץ חופשי מהמלמד — צילום, PDF או טקסט — ועליך לזהות באיזה סוג תוכן מדובר ולחלץ פריטים מובנים.
