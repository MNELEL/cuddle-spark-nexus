...
## 4. משימות שנפתחו בשיחה ועדיין פתוחות
...
**סטטוס מעודכן (23/7): פריט 1 (בלוגים מובנים) הושלם. פריט 2 (OCR→תעודות) התברר כבר קיים ומיושם. פריט 3 (איחוד AI Gateway) — בבדיקה נמצא חמישי (לא רק ארבעה) קבצים עם אותו בלוק fetch כפול: יש להוסיף גם `src/lib/ai-certificate.functions.ts` לרשימה. עדיין ברשימת העבודה הבאה.**
