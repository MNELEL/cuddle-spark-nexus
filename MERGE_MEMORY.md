...
11. Test suite ל-RLS ולהעתקת תלמידים — ❌ טרם בוצע, דורש הקמת תשתית טסטים
12. חיבור ה-trial הקיים למסך הרשמה + gate לתוכן חינמי — ❌ טרם בוצע
...
## 13. עמוד הגדרות מאוחד — מומש (10/8/2026)

מטרה: איגוד כל הגדרות המשתמש במקום אחד נגיש וקבוע, אחרי שהתגלה ש-`SecuritySettings`, `ReminderPreferencesCard` ומיתוג המוסד ישבו קודם מפוזרים בתוך `/toolkit` בלי כפתור ניווט ברור, וסטטוס תקופת הניסיון לא הוצג בשום מקום קבוע.

### 13.1 מיקום וניווט

- **Route קיים:** `src/routes/_authenticated.settings.index.tsx` (העמוד הראשי תחת `/settings`).
- **Header ראשי:** נוסף קישור "הגדרות" בניווט העליון (`src/routes/_authenticated.tsx`), ליד ארגז כלים / ספרייה / תובנות — כך שההגדרות נגישות מכל מסך מוביל בממשק.

### 13.2 תוכן העמוד

העמוד מחולק לכרטיסים ברורים:

1. **מיתוג (Brand)** — קישור / embed לעמוד `/settings/brand` הקיים (`src/routes/_authenticated.settings.brand.tsx`). העמוד עצמו לא הוזז פיזית; ההגדרות המוסדיות והתצוגה המקדימה נשארו במקומן.
2. **אבטחה (`SecuritySettings`)** — נעילת PIN, שינוי קוד, כיבוי נעילה, עם כפתור Retry ומצב שגיאה ממוקד (ראה סעיף 11.1).
3. **תזכורות (`ReminderPreferencesCard`)** — העדפות תזכורות ומיילים, עם עדכון אופטימי ו-rollback בשמירה (ראה סעיף 11.1).
4. **מצב ניסיון / מנוי** — כרטיס חדש המשתמש ב-`getMyTrialStatus` ומציג את מספר הימים שנותרו, תאריך סיום תקופת הניסיון, ומצב אישור/הרחבה. נועד לחבר את הטיפול הקיים ב-trial לממשק קבוע (סוגר פער שהתועד בסעיף 10.1 / 10.2).

### 13.3 החלטות תיעוד ושמירה על יציבות

- **לא הוזז ולא נמחק:** תוכן ההגדרות הקיים ב-`/toolkit` (לשוניות אבטחה, תזכורות, מסמכים) נשאר במקומו ללא שינוי. נמנעים משבירת קישורים קיימים או של command palette.
- **Command palette:** קיצור `Ctrl+K` עדיין מצביע ל-`/settings/brand` ותקין — לא השתנה.
- **הפרדת אחריות:** `/settings` הוא מרכז הגדרות משתמש (אבטחה, תזכורות, מנוי, מיתוג); `/toolkit` נשאר כארגז כלים פדגוגי/תפעולי (לוח, פעמונים, מחוללים, מסמכים).
