...
## 7. Class-Flow (Base44) — פיצ'רים חדשים שהתגלו (23/7, מהדפלוי החי)

נבדק ישירות מול הדפלוי החי (`https://classmanager-scoll-nm.base44.app/`, שם מסחרי 'Class-Flow') ומול קלון GitHub עדכני של `classflow` (בניגוד ל-Harmony Hub, כאן ה-GitHub מסתנכרן היטב עם הפרויקט החי — כל 51 העמודים מה-sitemap קיימים בריפו). התגלו 6 פיצ'רים חדשים שלא זוהו בהשוואה המקורית ואינם קיימים ב-Harmony Hub:

1. **Exam Scanner** (`ExamScannerPage.jsx`, 277 שורות) — AI מנקד מבחנים סרוקים בכתב-יד: מזהה שם תלמיד מהכותרת, מנקד לפי מחוון, שומר תוצאה לכל תלמיד. שונה בהיקף מה-OCR הקיים ב-Harmony Hub (שמזהה ציונים קיימים מרשימה, לא מנקד מבחן מלא).
2. **Analytics** (`AnalyticsPage.jsx`, 452 שורות) — דשבורד מגמות ציונים: גרפי קו/עמודות/radar לפי מקצוע ותלמיד, סדרי עדיפות מקצועות (נשמר ב-localStorage בצד הלקוח).
3. **Bell Schedule** (`BellSchedulePage.jsx`, 440 שורות) — מתזמן פעמוני שיעורים אוטומטי עם צלילים מותאמים (פעמון קלאסי/צלצול/דיגיטלי/מנגינות), הפעלה בזמן אמת לפי לוח יומי.
4. **Weekly Schedule** (`WeeklySchedulePage.jsx`, 503 שורות) — לא נבדק לעומק.
5. **Teacher Insights** (`TeacherInsightsPage.jsx`, 154 שורות) — לא נבדק לעומק.
6. **Student View** (`StudentViewPage.jsx`, 56 שורות) — לא נבדק לעומק.

**סטטוס: זוהו בלבד, טרם הושוו לעומק (מלבד Exam Scanner/Analytics/Bell Schedule שנבדקו ברמת קוד) וטרם הוחלט אם/איך לייבא ל-Harmony Hub. ברשימת המעקב לעתיד.**
